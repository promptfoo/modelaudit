...
class NumPyScanner(BaseScanner):
    ...
    def __init__(self, config=None):
        ...
        # Security limits
        self.max_array_bytes = self.config.get("max_array_bytes", 1024 * 1024 * 1024)  # 1GB
    ...
    def _validate_array_dimensions(self, shape: tuple) -> None:
        ...
        if len(shape) > self.max_dimensions:
            raise ValueError(f"Too many dimensions: {len(shape)} (max: {self.max_dimensions})")
        ...
        for i, dim in enumerate(shape):
            ...
            if dim > self.max_dimension_size:
                raise ValueError(f"Dimension {i} too large: {dim} (max: {self.max_dimension_size})")
    ...
    def _validate_dtype(self, dtype) -> None:
        ...
        if dtype.itemsize > self.max_itemsize:
            raise ValueError(f"Itemsize too large: {dtype.itemsize} bytes (max: {self.max_itemsize})")
    ...
    def _calculate_safe_array_size(self, shape: tuple, dtype) -> int:
        ...
        for dim in shape:
            ...
            if total_elements > max_elements // max(dim, 1):
                raise ValueError(f"Array size would overflow: shape={shape}, dtype={dtype}")
