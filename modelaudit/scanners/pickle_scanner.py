...
def _decode_string_to_bytes(s: str) -> list[tuple[str, bytes]]:
    ...
    try:
        ...
        if "=" not in s:
            padding_needed = (-len(s)) % 4
            if padding_needed in (1, 2):
                base64_candidate = s + ("=" * padding_needed)

        # Must be reasonable length and proper base64 format
        if (
            16 <= len(s) <= 10000  # Reasonable length bounds
            and len(base64_candidate) % 4 == 0
            and re.fullmatch(r"[A-Za-z0-9+/]+=*", base64_candidate)  # Proper base64 chars with padding
            and base64_candidate.count("=") <= 2  # At most 2 padding chars
            and not base64_candidate.replace("=", "").endswith("=")  # Padding only at end
        ):
