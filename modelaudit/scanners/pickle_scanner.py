...
class PickleScanner(BaseScanner):
    ...
    def scan(self, path: str) -> ScanResult:
        ...
        # Try fickling analysis first if available for enhanced security detection
        if FICKLING_AVAILABLE and self._get_bool_config("use_fickling", True):
            try:
                fickling_result = self._run_fickling_analysis(path, result)
                if fickling_result is not None:
                    # Fickling analysis completed - enhance with our comprehensive analysis
                    result.metadata["fickling_enhanced"] = True
                    self._enhance_result_with_comprehensive_analysis(result, path)
                    return result
            except Exception as e:
                # Fickling failed - add warning but continue with comprehensive analysis
                result.metadata["fickling_error"] = str(e)
                result.metadata["fickling_fallback"] = True
                result.add_check(
                    name="Fickling Analysis",
                    passed=False,
    ...
    def _run_fickling_analysis(self, path: str, result: ScanResult) -> Optional[bool]:
        ...
        try:
            ...
            result.add_check(
                ...
                passed=False,
                message=f"Fickling detected malicious content: {e}",
                severity=IssueSeverity.CRITICAL,
                location=path,
                details={
    ...
    def _map_fickling_severity(self, fickling_severity: Severity) -> IssueSeverity:
        ...
        if fickling_severity == Severity.LIKELY_SAFE:
            return IssueSeverity.INFO
        elif fickling_severity in (Severity.POSSIBLY_UNSAFE, Severity.SUSPICIOUS):
            return IssueSeverity.WARNING
    ...
    def _enhance_result_with_comprehensive_analysis(self, result: ScanResult, path: str) -> None:
