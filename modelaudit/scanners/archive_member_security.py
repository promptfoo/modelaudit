...
def _resolve_getattr_call_names(
    ...
    *,
    ...
    getattr_accessor_names = frozenset(
        ...
        for accessor_root in (normalized_helper_name.rsplit(".", maxsplit=1)[0],)
        if _is_ctypes_library_loader_object_root(accessor_root)
    )
    getattribute_accessor_names = frozenset(
        ...
        for accessor_root in (normalized_helper_name.rsplit(".", maxsplit=1)[0],)
        if not _is_ctypes_library_loader_object_root(accessor_root)
    )
    ...
    if accessor_names:
        ...
        if attr_name is None and getattr_accessor_names:
            return frozenset(f"{target_root}.{_CTYPES_DYNAMIC_LIBRARY_NAME}" for target_root in getattr_accessor_names)
        if attr_name is None:
            return None
    ...
    if attr_name is None:
        loader_roots = frozenset(
            target_root for target_root in resolved_target_roots if _is_ctypes_library_loader_object_root(target_root)
        )
        return (
            frozenset(f"{resolved_target_root}.{_CTYPES_DYNAMIC_LIBRARY_NAME}" for resolved_target_root in loader_roots)
            or None
        )
    return _apply_aliases_to_names(
        frozenset(f"{resolved_target_root}.{attr_name}" for resolved_target_root in resolved_target_roots),
...
class _HighRiskPythonCallVisitor(ast.NodeVisitor):
    ...
    def _call_preserves_ctypes_loader_init(
        self, node: ast.Call, *, self_name: str | None, class_scope: _AliasScope, super_preserves_init: bool
    ) -> bool:
        if (
    ...
    def _node_preserves_ctypes_loader_init(
        self, node: ast.AST, *, self_name: str | None, class_scope: _AliasScope, super_preserves_init: bool
    ) -> bool:
        if isinstance(node, ast.Call) and self._call_preserves_ctypes_loader_init(
            node, self_name=self_name, class_scope=class_scope, super_preserves_init=super_preserves_init
        ):
            return True
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef, ast.Lambda)):
            return False
        if isinstance(node, ast.If):
            if self._node_preserves_ctypes_loader_init(
                node.test,
                self_name=self_name,
                class_scope=class_scope,
                super_preserves_init=super_preserves_init,
            ):
                return True
        ...
        return any(
            self._node_preserves_ctypes_loader_init(
                child,
                self_name=self_name,
                class_scope=class_scope,
                super_preserves_init=super_preserves_init,
    ...
    def _class_preserves_ctypes_loader_init(self, node: ast.ClassDef, class_scope: _AliasScope) -> bool:
        ...
        return self._body_preserves_ctypes_loader_init(
            ...
            self_name=self_name,
            class_scope=class_scope,
            super_preserves_init=bool(first_base_loader_types),
        )
