...
class TextScanner(BaseScanner):
    ...
    @classmethod
    def _documentation_line_has_expression(cls, line: bytes, expression_type: type[ast.expr]) -> bool:
        return any(
            isinstance(node, expression_type)
            for statement in cls._documentation_python_statements(line)
    ...
    def _documentation_line_is_code_shaped(cls, line: bytes, position: int) -> bool:
        ...
        return (
            ...
            or cls._documentation_line_has_expression(line, ast.Call)
            or cls._documentation_line_has_expression(line, ast.Lambda)
    ...
    def _documentation_finding_is_actionable(cls, payload: bytes, finding: dict[str, Any]) -> bool:
        ...
        if line_parts is not None:
            if cls._documentation_shell_comment_before_position(*line_parts):
                return False
            if cls._documentation_line_is_code_shaped(*line_parts):
                return True
        position = finding.get("position")
        if not isinstance(position, int) or position < 0 or position > len(payload):
            return False
