...
def format_sarif_output(
    ...
    sarif_output = {
        ...
        "version": "2.1.0",
        "runs": [
            _create_run(audit_result, scan_paths, verbose)
        ]
    }
...
def _create_run(
    ...
    run = {
        ...
        "automationDetails": {
            "description": {
                "text": "ModelAudit security scan for ML model files"
            },
    ...
    for issue in issues:
        ...
        if rule_id not in seen_rules:
            ...
            rule = {
                ...
                "shortDescription": {
                    "text": _get_rule_short_description(issue)
                },
                "fullDescription": {
                    "text": _get_rule_full_description(issue)
                },
    ...
    for idx, issue in enumerate(issues):
        ...
        if issue.location:
            location = {
                "physicalLocation": {
                    "artifactLocation": {
                        "uri": _normalize_path_to_uri(issue.location),
                        "uriBaseId": "%SRCROOT%"
                    }
                }
            }
        ...
        fingerprint = hashlib.sha256(
            f"{issue.message}{issue.location or ''}{issue.severity}".encode()
        ).hexdigest()[:16]
...
def _create_artifacts(audit_result: ModelAuditResultModel) -> list[dict[str, Any]]:
    ...
    for asset in audit_result.assets:
        artifact = {
            "location": {
                "uri": _normalize_path_to_uri(asset.path),
                "uriBaseId": "%SRCROOT%"
            },
...
def _get_rule_name(issue) -> str:
    ...
    return issue.message.split(':')[0] if ':' in issue.message else issue.message[:50]
