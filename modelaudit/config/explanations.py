...
def get_cve_2025_49655_explanation(issue_type: str) -> str:
    ...
    CVE-2025-49655 (CVSS 9.8 CRITICAL): TorchModuleWrapper layers in Keras 3.11.0-3.11.2
    and prereleases of 3.11.3 call torch.load(weights_only=False) in from_config(), enabling
    arbitrary code execution via pickle deserialization. Fixed in Keras 3.11.3.
    """
    explanations = {
        "torch_module_wrapper": (
            "CVE-2025-49655: TorchModuleWrapper layers in Keras 3.11.0-3.11.2 and prereleases "
            "of 3.11.3 call torch.load(weights_only=False) during from_config() deserialization, which allows "
