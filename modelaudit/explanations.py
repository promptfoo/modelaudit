...
DANGEROUS_IMPORTS = {
    "os": (
        "The 'os' module provides direct access to operating system functions, allowing execution of arbitrary system commands, "
        "file system manipulation, and environment variable access. Malicious models can use this to compromise the host system, "
    ...
    "sys": (
        ...
        "modify the Python runtime, access command-line arguments, or manipulate the module import system to load malicious code."
    ),
    "subprocess": (
        "The 'subprocess' module allows spawning new processes and executing system commands. This is a critical security risk "
        "as it enables arbitrary command execution on the host system."
    ),
    "eval": (
        "The 'eval' function executes arbitrary Python code from strings. This is extremely dangerous as it allows dynamic "
        "code execution, potentially running any malicious code embedded in the model."
    ),
    "exec": (
    ...
    "__import__": (
        "The '__import__' function dynamically imports modules at runtime. Attackers can use this to load malicious modules or bypass import restrictions."
    ),
    "importlib": (
        "The 'importlib' module provides programmatic module importing capabilities. It can be used to dynamically load "
        "malicious code or bypass security controls."
    ),
    "pickle": (
        "Nested pickle operations (pickle.load/loads within a pickle) can indicate attempts to obfuscate malicious payloads or create multi-stage attacks."
    ),
    ...
    "ctypes": (
        "The 'ctypes' module provides low-level system access through foreign function interfaces. It can bypass Python's "
        "safety features and directly manipulate memory or call system libraries."
    ),
    "pty": (
        "The 'pty' module provides pseudo-terminal utilities. The 'spawn' function can be used to create interactive shells, "
        "potentially giving attackers remote access."
    ),
    "platform": "Functions like 'platform.system' or 'platform.popen' can be used for system reconnaissance or command execution.",
    "shutil": (
        "The 'shutil' module provides high-level file operations. Functions like 'rmtree' can recursively delete directories, potentially causing data loss."
    ),
    ...
    "operator.attrgetter": (
        "The 'attrgetter' function can be used to access object attributes dynamically, potentially bypassing access controls or reaching sensitive data."
    ),
    ...
    "dill": (
        "The 'dill' module extends pickle's capabilities to serialize almost any Python object, including lambda functions "
        "and code objects. This significantly increases the attack surface for code execution."
    ),
}
...
DANGEROUS_OPCODES = {
    "REDUCE": (
        "The REDUCE opcode calls a callable with arguments, effectively executing arbitrary Python functions. This is the "
        "primary mechanism for pickle-based code execution attacks through __reduce__ methods."
    ),
    "INST": (
        "The INST opcode instantiates objects by calling their class constructor. Malicious classes can execute code in __init__ methods during unpickling."
    ),
    ...
    "NEWOBJ_EX": (
        "The NEWOBJ_EX opcode is an extended version of NEWOBJ with additional capabilities for creating objects, potentially executing initialization code."
    ),
    "BUILD": "The BUILD opcode updates object state and can trigger code execution through __setstate__ or __setattr__ methods.",
    "STACK_GLOBAL": (
        "The STACK_GLOBAL opcode imports modules and retrieves attributes dynamically. Outside ML contexts, this often "
        "indicates attempts to access dangerous functionality."
    ),
...
PATTERN_EXPLANATIONS = {
    "base64_payload": (
        "Base64-encoded data in models often conceals malicious payloads. Legitimate ML models rarely need encoded strings "
        "unless handling specific data formats."
    ),
    "hex_encoded": (
        "Hexadecimal-encoded strings (\\x00 format) can hide malicious code or data. This obfuscation technique is commonly used to evade detection."
    ),
    "lambda_layer": (
        "Lambda layers in Keras/TensorFlow can contain arbitrary Python code that executes during model inference. Unlike "
        "standard layers, these can perform system operations beyond tensor computations."
    ),
    "executable_in_zip": (
        "Executable files (.exe, .sh, .bat, etc.) within model archives are highly suspicious. ML models should only contain "
        "weights and configuration, not executables."
    ),
    "dissimilar_weights": (
        "Weight vectors that are completely dissimilar to others in the same layer may indicate injected malicious data masquerading as model parameters."
    ),
    "outlier_neurons": (
        "Neurons with weight distributions far outside the normal range might encode hidden functionality or backdoors rather than learned features."
    ),
    ...
    "manifest_name_mismatch": (
        "Model names in manifests that don't match expected patterns may indicate tampered or malicious models trying to impersonate legitimate ones."
    ),
