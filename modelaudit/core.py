...
def scan_model_streaming(
    ...
    **kwargs: Any,
    ...
    file_hashes: list[str] = []
    files_processed = 0
    max_total_size = kwargs.get("max_total_size", 0)
    limit_reached = False
    ...
    try:
        ...
        if limit_reached:
            logger.warning("Streaming scan terminated early due to total size limit")
            _add_issue_to_model(
                results,
                "Scan terminated early due to total size limit",
                severity=IssueSeverity.INFO.value,
                location=str(scan_root) if scan_root is not None else None,
                details={"max_total_size": max_total_size},
            )
