...
def _record_incomplete_dvc_resolution(
    ...
    if not resolution.analysis_incomplete or (
        resolution.incomplete_reason == DVC_OUTPUT_LIMIT_EXCEEDED_REASON and not resolution.unresolved_outputs
    ):
        return
...
def _record_dvc_output_limit_incomplete(
    ...
    """Record capped DVC outputs that were not independently covered."""
    ...
    _add_issue_to_model(
        ...
        issue_type=DVC_OUTPUT_LIMIT_EXCEEDED_REASON,
    )
...
def _dvc_omitted_outputs_covered_by_directory_walk(
    ...
    covered_directories: set[str],
    *,
    skip_file_types: bool,
    metadata_scanner_available: bool,
    scanner_selection_extensions: frozenset[str] | None,
) -> bool:
    """Return whether actual directory scans covered every capped DVC target."""

    def is_covered(target: Path) -> bool:
        target_str = str(target)
        if target.is_file():
            return target_str in covered_paths
        if not target.is_dir() or target_str not in covered_directories:
            return False

        walk_errors: list[OSError] = []
        for root, dirs, files in os.walk(target, followlinks=False, onerror=walk_errors.append):
            if str(Path(root).resolve()) not in covered_directories:
                return False
            for directory_name in dirs:
                try:
                    ...
                    return False
                if resolved_directory not in covered_directories:
                    return False
            for filename in files:
                ...
                try:
                    ...
                    return False
                if resolved_file not in covered_paths:
                    return False
    ...
    return dvc_omitted_outputs_covered(
        ...
        resolution,
        is_covered,
        coverage_budget=len(covered_paths) + len(covered_directories),
    )
    ...
    **kwargs: Any,
    ...
    try:
        ...
        if os.path.isdir(path):
            ...
            for root, dirs, files in os.walk(
                ...
                for file in sorted(files):
                    ...
                    if file.lower().endswith(".dvc"):
                        ...
                        if dvc_resolution.output_limit_analysis_incomplete:
                            pending_dvc_output_limit_checks.append((file_path, dvc_resolution))
                        if dvc_resolution.analysis_incomplete and (
                            dvc_resolution.incomplete_reason != DVC_OUTPUT_LIMIT_EXCEEDED_REASON
                            or dvc_resolution.unresolved_outputs
                        ):
            ...
            if is_dvc_pointer:
                single_dvc_resolution = resolve_dvc_file_status(path)
                _record_incomplete_dvc_resolution(results, scan_metadata, path, single_dvc_resolution)
                if single_dvc_resolution.analysis_incomplete and (
                    single_dvc_resolution.incomplete_reason != DVC_OUTPUT_LIMIT_EXCEEDED_REASON
                    or single_dvc_resolution.unresolved_outputs
                ):
                    aggregate_hash_complete = False
            ...
            scanned_dvc_directories: set[str] = set()
            ...
            for _idx, target in enumerate(target_files):
                ...
                if os.path.isdir(target):
                    ...
                    for root, _dirs, _files in os.walk(target, followlinks=False):
                        scanned_dvc_directories.add(str(Path(root).resolve()))
            ...
            if (
                single_dvc_resolution is not None
                and single_dvc_resolution.output_limit_analysis_incomplete
                and not _dvc_omitted_outputs_covered_by_directory_walk(
                    path,
                    single_dvc_resolution,
                    scanned_dvc_paths,
                    scanned_dvc_directories,
                    skip_file_types=skip_file_types,
                    metadata_scanner_available=metadata_scanner_available,
                    scanner_selection_extensions=scanner_selection_extensions,
                )
