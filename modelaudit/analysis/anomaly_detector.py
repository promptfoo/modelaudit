    ...
    def __init__(self):
        ...
        self.layer_profiles = {
            "conv2d_weights": StatisticalProfile(
                mean=0.0, std=0.05, min_val=-0.3, max_val=0.3,
                percentiles={1: -0.15, 5: -0.1, 25: -0.05, 50: 0.0, 75: 0.05, 95: 0.1, 99: 0.15},
                skewness=0.0, kurtosis=3.0, entropy=7.5, zero_ratio=0.01, sparsity=0.05
            ),
            "dense_weights": StatisticalProfile(
                mean=0.0, std=0.1, min_val=-0.5, max_val=0.5,
                percentiles={1: -0.25, 5: -0.2, 25: -0.1, 50: 0.0, 75: 0.1, 95: 0.2, 99: 0.25},
                skewness=0.0, kurtosis=3.0, entropy=7.6, zero_ratio=0.02, sparsity=0.1
            ),
            "attention_weights": StatisticalProfile(
                mean=0.0, std=0.02, min_val=-0.1, max_val=0.1,
                percentiles={1: -0.05, 5: -0.04, 25: -0.02, 50: 0.0, 75: 0.02, 95: 0.04, 99: 0.05},
                skewness=0.0, kurtosis=3.0, entropy=7.3, zero_ratio=0.001, sparsity=0.01
            ),
            "embedding_weights": StatisticalProfile(
                mean=0.0, std=0.1, min_val=-0.5, max_val=0.5,
                percentiles={1: -0.3, 5: -0.2, 25: -0.1, 50: 0.0, 75: 0.1, 95: 0.2, 99: 0.3},
                skewness=0.0, kurtosis=3.0, entropy=7.7, zero_ratio=0.001, sparsity=0.02
            ),
            "batch_norm_weights": StatisticalProfile(
                mean=1.0, std=0.1, min_val=0.5, max_val=1.5,
                percentiles={1: 0.8, 5: 0.85, 25: 0.95, 50: 1.0, 75: 1.05, 95: 1.15, 99: 1.2},
                skewness=0.0, kurtosis=3.0, entropy=6.5, zero_ratio=0.0, sparsity=0.0
            )
        }
    ...
    def compute_statistical_profile(self, data: np.ndarray) -> StatisticalProfile:
        ...
        if data.size == 0:
            return StatisticalProfile(
                ...
                percentiles={p: 0 for p in [1, 5, 25, 50, 75, 95, 99]},
                skewness=0, kurtosis=0, entropy=0, zero_ratio=1, sparsity=1
            )
        ...
        # Percentiles
        percentiles = {
            p: np.percentile(flat_data, p) for p in [1, 5, 25, 50, 75, 95, 99]
        }
        ...
        return StatisticalProfile(
            ...
            percentiles=percentiles, skewness=skewness, kurtosis=kurtosis,
            entropy=entropy, zero_ratio=zero_ratio, sparsity=sparsity
        )
    ...
    def _check_suspicious_patterns(self, data: np.ndarray) -> List[Dict[str, any]]:
        ...
        if self._has_repeating_patterns(flat_data):
            suspicious.append({
                ...
                "severity": "medium",
                "details": "Unusual repeating patterns detected"
            })
