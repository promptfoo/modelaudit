...
from modelaudit.context.unified_context import UnifiedMLContext, ModelArchitecture
from modelaudit.analysis.entropy_analyzer import EntropyAnalyzer
from modelaudit.analysis.semantic_analyzer import SemanticAnalyzer, CodeRiskLevel
...
from modelaudit.utils.code_validation import validate_python_syntax, is_code_potentially_dangerous
...
class IntegratedAnalyzer:
    ...
    def analyze_suspicious_pattern(self, 
                                 pattern: str,
                                 pattern_type: str,
                                 context: UnifiedMLContext,
                                 raw_data: Optional[bytes] = None,
                                 code_snippet: Optional[str] = None) -> IntegratedAnalysisResult:
        ...
        total_weight = sum(self.signal_weights[k] for k in signals.keys())
    ...
    def _analyze_anomalies(self, pattern: str, context: UnifiedMLContext) -> Dict[str, Any]:
        ...
        return {
            "confidence": max(0, confidence_safe),
            "reasoning": reasoning,
            "anomalies_found": len(reasoning) > 0
        }
    
    def _analyze_framework_patterns(self, pattern: str, pattern_type: str, 
                                  context: UnifiedMLContext) -> Dict[str, Any]:
        ...
        if framework:
            ...
            if self.framework_kb.should_skip_pattern_for_framework(
                pattern, framework, {"filename": context.file_path}
            ):
                confidence_safe = 0.95
    ...
    def _get_recommendations(self, pattern: str, pattern_type: str, 
                           context: UnifiedMLContext) -> List[str]:
        ...
        if context.primary_framework:
            recommendations.append(
                f"Use {context.primary_framework}-specific security tools"
            )
