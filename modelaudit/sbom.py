...
def generate_sbom_with_path_mapping(
    paths: Iterable[str], results_dict: dict[str, Any], path_mappings: dict[str, str]
) -> str:
    """
    Generate SBOM with support for URL-to-file path mapping.

    This function handles cases where the original paths are URLs that have been
    downloaded to local files. The path_mappings dictionary maps original URLs
    to their downloaded file paths for accurate SBOM generation.

    Args:
        paths: Original input paths (may include URLs)
        results_dict: Scan results dictionary
        path_mappings: Dictionary mapping original URLs to downloaded file paths
    ...
    file_metadata = results_dict.get("file_metadata", {})
    issues = results_dict.get("issues", [])

    # Group issues by their file location for efficient lookup
    issues_by_location: dict[str, list[dict[str, Any]]] = {}
    for issue in issues:
    ...
    for input_path in paths:
        # Resolve the actual file path using mappings
        actual_path = path_mappings.get(input_path, input_path)

        # Skip if the actual path doesn't exist (failed download or URL issue)
        if not os.path.exists(actual_path):
            continue

        # Use the actual file path for metadata and issues
        if os.path.isdir(actual_path):
            # For directories, scan all files
            for root, _dirs, files in os.walk(actual_path):
                for f in files:
                    fp = os.path.join(root, f)
                    metadata = file_metadata.get(fp)
                    # Get issues for this specific file
                    file_issues = issues_by_location.get(fp, [])
                    component = _component_for_file(fp, metadata or {}, file_issues)
                    bom.components.add(component)
        else:
            # For single files, use the actual path but get metadata/issues
            # using either the original path or actual path (whichever has data)
            metadata = file_metadata.get(actual_path) or file_metadata.get(input_path)
            file_issues = issues_by_location.get(actual_path, []) or issues_by_location.get(input_path, [])
            component = _component_for_file(actual_path, metadata or {}, file_issues)
            bom.components.add(component)
