...
def scan_command(
        ...
        try:
            ...
            if verbose:
                logger.warning(
                    ...
                    "Install it with 'pip install python-dotenv' to use .env files."
                )
    ...
    if format == "text" and not output:
        ...
        click.echo("\n".join(header))
        click.echo(f"Paths to scan: {style_text(', '.join(paths), fg='green')}")
        if blacklist:
            click.echo(
        ...
        # Check if it's a DVC file
            ...
            try:
                ...
                if dvc_paths:
                    ...
                    # If DVC file can't be resolved, still add it to scan list
                    # This allows scanning of the DVC file itself or reporting if it doesn't exist
                    click.echo(f"Warning: Could not resolve DVC file: {path}", err=True)
                    expanded_paths.append(path)
            except Exception as e:
                click.echo(f"Error resolving DVC file {path}: {e!s}", err=True)
                # Still add the DVC file to scan list to report error properly
                expanded_paths.append(path)
