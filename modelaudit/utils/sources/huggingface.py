...
def _remote_safetensors_index_failure_result(
    repo_id: str,
    index_filename: str,
    revision: str,
    details: dict[str, Any],
) -> Any:
    """Return an operational result for index failures with no provable shard scope."""
    from ...core_results import mark_operational_scan_error
    from ...scanner_results import ScanResult, mark_inconclusive_scan_result
    from ...scanners.base import IssueSeverity
    ...
    result.metadata.update(
        {
            ...
            "hf_repo_id": repo_id,
            "hf_revision": revision,
            "hf_filename": index_filename,
            "remote_bytes_transferred": bytes_transferred,
            "analysis_scope": "safetensors_index_reconciliation",
            "remote_index_only": True,
            "remote_index_reconciliation": dict(details),
        }
    )
    mark_inconclusive_scan_result(result, _HF_SAFETENSORS_INDEX_RECONCILIATION_REASON)
    mark_operational_scan_error(result, _HF_SAFETENSORS_INDEX_RECONCILIATION_REASON)
    result.add_check(
        name="Hugging Face SafeTensors Index Reconciliation",
        passed=False,
        message="Unable to prove the shard scope of a remote SafeTensors index failure.",
        severity=IssueSeverity.INFO,
        location=source_path,
        details={
            **details,
...
def _detect_huggingface_text_owner_embedded_binary_route(
    ...
    def detect_sample_route(sample: bytes) -> str | None:
        ...
        if flax_state is True and route_is_active("flax_msgpack"):
            return "flax_msgpack"
        if (
            _starts_with_non_ascii_length_delimited_proto_prefix(sample)
            or _has_bounded_protobuf_model_text_candidate_signal_bytes(sample)
        ) and route_is_active(PROTOBUF_MODEL_CANDIDATE_FORMAT):
            return PROTOBUF_MODEL_CANDIDATE_FORMAT
        if _looks_like_proto0_or_1_pickle(sample, sample_is_prefix=True) and route_is_active("pickle"):
            return "pickle"
        ...
        if detected_format != "unknown" and route_is_active(detected_format):
    ...
    if post_prefix_route is not None and route_is_active(post_prefix_route):
    ...
    tail_route = detect_sample_route(tail)
    return tail_route if tail_route is not None and route_is_active(tail_route) else None
...
def _select_huggingface_model_files(
    ...
    *,
    ...
    active_content_route_scanner_ids = _active_hf_content_route_scanner_ids(
    ...
    probe_budget = _HuggingFaceProbeBudget(
        ...
        deadline=deadline,
        active_scanner_ids=active_content_route_scanner_ids,
    )
...
def _allows_safetensors_index_expansion(
    ...
    *,
    ...
    """Return whether remote selection authorizes SafeTensors shard expansion."""
    if include_all_files:
        return True
    if scannable_scanner_ids is not None:
        return "safetensors" in {str(scanner_id).lower() for scanner_id in scannable_scanner_ids}
    if scannable_extensions is not None:
        return ".safetensors" in {str(extension).lower() for extension in scannable_extensions}
    return True
...
def _validate_remote_safetensors_indexes(
    ...
    *,
    ...
    for index_file in relevant_index_files:
        ...
        unreferenced_siblings = sorted(
            sibling
            for family, referenced_targets in target_families.items()
            for sibling in repo_family_files.get(family, set()) - referenced_targets
        )
...
def _active_hf_content_route_scanner_ids(
    selected_route_scanner_ids: set[str] | None,
    selected_route_formats: set[str] | None,
) -> frozenset[str] | None:
    """Return the active scanner policy shared by all bounded HF content probes."""
    if selected_route_scanner_ids is not None:
        return frozenset(selected_route_scanner_ids)
    if selected_route_formats is None:
        return None
    return _hf_route_scanner_ids_for_formats(frozenset(selected_route_formats))
...
def _select_streamable_hf_files(
    ...
    *,
    ...
    deadline: float | None = None,
    probe_budget: _HuggingFaceProbeBudget | None = None,
    defer_safetensors_index_validation: bool = False,
    ...
    if sniff_renamed_files:
        ...
        if probe_budget is None:
            ...
            probe_budget.active_scanner_ids = active_content_route_scanner_ids
        for file_name in _streamable_hf_content_probe_candidates(
            ...
            if (
                _is_safetensors_index_file(file_name)
                and defer_safetensors_index_validation
                and (
                    active_content_route_scanner_ids is None
                    or not active_content_route_scanner_ids.difference({"safetensors"})
                )
            ):
                continue
            if inspected_files >= _HF_CONTENT_SNIFF_MAX_FILES:
...
def _should_cleanup_hf_streaming_context_file(
    ...
    *,
    ...
    if not cache_dir_owned or download_path is None:
        return False
...
def plan_huggingface_streaming_download(
    ...
    *,
    ...
    plan_probe_budget = _HuggingFaceProbeBudget(
        remaining_bytes=(
            min(_HF_CONTENT_SNIFF_MAX_TOTAL_BYTES, size_limit)
            if size_limit is not None
    ...
    selection = _select_streamable_hf_files(
        ...
        deadline=deadline,
        probe_budget=plan_probe_budget,
        defer_safetensors_index_validation=defer_safetensors_index_validation,
    )
    model_files = selection.filenames
    ...
    minimum_selected_bytes = (
        plan_probe_budget.transferred_bytes
        + minimum_safetensors_read_bytes
        + sum(selected_sizes.get(filename, 0) for filename in size_preflight_files)
    )
...
def download_model_streaming(
    ...
    *,
    ...
    Args:
        ...
        _cache_dir_owned: Internal marker that the caller owns the complete disposable cache root
    ...
    try:
        ...
        safetensors_header_files: set[str] = set()
        remaining_probe_bytes = max(_HF_CONTENT_SNIFF_MAX_TOTAL_BYTES - plan.probe_bytes_transferred, 0)
        if size_limit is not None:
            remaining_probe_bytes = min(remaining_probe_bytes, max(size_limit - plan.probe_bytes_transferred, 0))
