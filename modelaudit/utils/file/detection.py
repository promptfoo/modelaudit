...
_LEGAL_TEXT_ENCODED_EXECUTION_RE = re.compile(
    rb"(?:"
    rb"\b(?:eval|exec|__import__)\s*\(|"
    rb"\bos\s*\.\s*system\s*\(|"
    rb"\bsubprocess\s*\.\s*(?:call|check_call|check_output|getoutput|getstatusoutput|popen|run)\s*\("
    rb")",
    re.IGNORECASE,
)
...
def _detect_bounded_xml_model_file_format(file_path: Path, file_size: int, prefix: bytes) -> str:
    """Recognize a structured XML model before text or serialized fallbacks."""
    if not _could_be_xml_prefix(prefix):
        return "unknown"
    xml_prefix = read_magic_bytes(str(file_path), min(file_size, _XML_MODEL_SIGNATURE_READ_BYTES))
    return _detect_xml_model_format(
        xml_prefix,
        sample_is_prefix=file_size > len(xml_prefix),
    )
...
_PICKLE_TRIVIAL_NO_ARGUMENT_PREFIX_BYTES: frozenset[int] = frozenset(
    value
    for value, opcode in _PICKLE_OPCODE_BY_BYTE.items()
    if opcode.name in PROTO0_1_TRIVIAL_LEADING_OPCODES and opcode.arg is None
)
...
def _is_legal_text_sidecar_name(path: Path, logical_name: str | None = None) -> bool:
    ...
    if extension not in LEGAL_TEXT_SIDECAR_EXTENSIONS:
        return False
    basename = filename[: -len(extension)] if extension else filename
    return basename in LEGAL_TEXT_SIDECAR_BASENAMES
...
def _decode_complete_utf8_plain_text(payload: bytes) -> str | None:
    try:
...
def _meaningful_pickle_events(
    ...
    *,
    ...
    if (
        ...
        and all(index == 0 for index, _name, _flag in meaningful)
    ):
        return []
    return meaningful
...
def _classify_bounded_pickle_candidate(
    ...
    *,
    ...
    while stream.tell() < len(sample):
        ...
        text_line_side_effects_only = True
        pre_inst = False
        protocol = False
        ...
        try:
            for opcode, argument, _position in _gen_pickle_probe_ops(stream):
                ...
                if not _apply_pickle_stack_effect(
                    ...
                    return incomplete_route(stream_start, pre_inst, protocol, text_line_side_effects_only, events)
                if opcode_name not in _PICKLE_TEXT_LINE_SIDE_EFFECT_OPCODES and (
                    events or opcode_name not in PROTO0_1_TRIVIAL_LEADING_OPCODES
                ):
                    text_line_side_effects_only = False

                if opcode_name in _BINARY_PICKLE_SECURITY_OPCODES:
                    events.append((opcode_index, opcode_name, meaningful_argument))
            ...
            route = incomplete_route(stream_start, pre_inst, protocol, text_line_side_effects_only, events)
...
def _is_plausible_pickle_candidate(payload: bytes, offset: int = 0) -> bool:
    ...
    if argument_size > 0:
        operand_end = offset + 1 + argument_size
        if operand_end > len(payload):
            return False
        return (
            opcode.name in _BINARY_PICKLE_SECURITY_OPCODES
            or operand_end == len(payload)
    ...
    if argument_size == -1:
        ...
        if opcode.name == "STRING":
            ...
            if len(value) < 2 or value[:1] not in {b"'", b'"'} or value[-1:] != value[:1]:
                return False
        return cursor == len(payload) or payload[cursor] in _PICKLE_OPCODE_BY_BYTE
    ...
    operand_end = offset + 1 + header_size + payload_size
    if payload_size < 0 or operand_end > len(payload):
        return False
    return (
        opcode.name in _BINARY_PICKLE_SECURITY_OPCODES
        or operand_end == len(payload)
...
def _has_trivial_pickle_prefix(payload: bytes, start: int, end: int) -> bool:
    ...
    for (
        ...
        if decoded:
            ...
            exact_weak_decoded_stream = (
                ...
                and offset == 0
                and all(
                    value == ord(".") or value in PROTO0_1_IGNORABLE_TRAILING_BYTES
    ...
    for raw_line in payload.splitlines():
        ...
        if encoded_line is not None:
            normalized_alphabetic_prose = (
                alphabetic_whitespace_is_prose
                and encoded_line != line
    ...
    for (
        ...
        for token, trust_weak_nonstop_suffix in _iter_encoded_route_tokens(
            ...
            if len(token) > maximum_encoded_length:
                zero_fill_token = (
                    alphabetic_offset_zero_only and bool(unpadded_token) and set(unpadded_token) == {ord("A")}
                )
                if not zero_fill_token:
                    return PICKLE_ROUTING_INCONCLUSIVE_FORMAT
...
def _legal_text_sidecar_route_from_bytes(
    ...
    *,
    ...
    if text is None:
        return None
    filename = _logical_basename_for_route(path, logical_name)
    if filename in MAIN_OWNED_LEGAL_TEXT_FILENAMES or _LEGAL_TEXT_SIGNAL_RE.search(text) is not None:
        return "text"
    return None
        ...
        with file_path.open("rb") as f:
            ...
            if format_result in {"gzip", "bzip2", "xz"}:
                ...
                if tar_route in {"nemo", NEMO_ROUTING_INCONCLUSIVE_FORMAT}:
                    return tar_route
            if format_result not in {"unknown", "pickle"}:
...
def detect_file_format_for_skip_filter(path: str, *, logical_name: str | None = None) -> str:
    ...
    with file_path.open("rb") as f:
        ...
        xml_format = _detect_bounded_xml_model_file_format(file_path, size, prefix)
        if xml_format != "unknown":
            return xml_format
