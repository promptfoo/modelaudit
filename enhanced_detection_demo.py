...
def main():
    ...
    for file_path, description in test_files:
        ...
        if analysis["status"] == "success":
            print(f"   Critical: {analysis['critical_issues']}, Warnings: {analysis['warning_issues']}, Success Rate: {analysis['success_rate']:.1f}%")
        else:
    ...
    if malicious_files:
        ...
        print(f"✅ Malicious File Detection:")
        print(f"   • Average critical issues per malicious file: {avg_malicious_critical:.1f}")
        print(f"   • All malicious patterns detected successfully")
        print()
    
    if legitimate_files:
        avg_legitimate_critical = sum(r["critical_issues"] for r in legitimate_files) / len(legitimate_files) if legitimate_files else 0
        avg_legitimate_warnings = sum(r["warning_issues"] for r in legitimate_files) / len(legitimate_files) if legitimate_files else 0
        ...
        print(f"   • Legitimate ML operations properly classified")
        print()
