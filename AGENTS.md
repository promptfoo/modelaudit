...
- Reuse shared fixture helpers for container formats. For PyTorch ZIP tests, prefer `tests.helpers.create_mock_pytorch_zip`; if you hand-roll a ZIP-backed `.pt`/`.pkl`, include minimal PyTorch markers (for example `version` and `byteorder`) so routing matches production.
