...
def create_sample_flax_checkpoint(path: str):
    ...
    checkpoint_data = {
        "params": {
            "transformer": {
                "embeddings": {
                    "token_embedding": np.random.randn(50257, 768)
                    .astype(np.float32)
                    .tobytes(),
                    "position_embedding": np.random.randn(1024, 768)
                    .astype(np.float32)
                    .tobytes(),
                },
                "encoder": {
                    "layer_0": {
                        "attention": {
                            "query": np.random.randn(768, 768)
                            .astype(np.float32)
                            .tobytes(),
                            "key": np.random.randn(768, 768)
                            .astype(np.float32)
                            .tobytes(),
                            "value": np.random.randn(768, 768)
                            .astype(np.float32)
                            .tobytes(),
                        },
                        "feed_forward": {
                            "dense": np.random.randn(768, 3072)
                            .astype(np.float32)
                            .tobytes(),
...
def create_suspicious_jax_checkpoint(path: str):
    ...
    suspicious_data = {
        "params": {
            "layer": np.random.randn(10, 10).tobytes() if HAS_NUMPY else b"\x00" * 400
        },
...
def demo_flax_msgpack_scanning():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        if create_sample_flax_checkpoint(flax_path):
            ...
            print(
                f"  🏗️  Model architecture: {result.metadata.get('model_architecture', 'unknown')}"
            )
            print(
                f"  🧮 Estimated parameters: {result.metadata.get('estimated_parameters', 0):,}"
            )
...
def demo_jax_checkpoint_scanning():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        print(f"  📊 Issues found: {len(result.issues)}")
        print(
            f"  📁 Checkpoint type: {result.metadata.get('checkpoint_type', 'unknown')}"
        )
        ...
        for dir_name, should_handle in test_dirs:
            ...
            print(
                f"  {dir_name}: {status} (expected: {should_handle}, got: {can_handle})"
            )
            ...
            "data": {
                ...
                "shape_info": {
                    "shape": [
                        ...
                        -100,
                        1000000000,
                    ],  # Negative and extremely large dims
                    "dtype": "float32",
                },
            },
        },
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        for i, threat in enumerate(threats, 1):
            ...
            if critical_issues or warning_issues:
                print(
                    f"  🚨 Threats detected: {len(critical_issues)} critical, {len(warning_issues)} warnings"
                )
                if critical_issues:
