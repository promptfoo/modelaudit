...
`scan_file` auto-detects PyTorch ZIP containers (archives containing `data.pkl` plus `version` / `byteorder` metadata), enumerates discovered pickle members (including members identified by content sniffing, not just extension), and combines per-member reports into a single container-level report with `metadata.container_type="pytorch_zip"`. Archive member count is capped at 10,000 entries; per-member pickles are capped at 512 MiB. Both limits are enforced by structured notices, not silent skips.
