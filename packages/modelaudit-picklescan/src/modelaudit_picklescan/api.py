...
def _base64_literal_value_has_suspicious_text(value: bytes) -> bool:
    ...
    for match in _BASE64_LITERAL_TEXT_TOKEN_RE.finditer(value):
        ...
        if candidate_count > _MAX_STORAGE_LITERAL_TEXT_CANDIDATES:
            return True
        token_segments = _base64_literal_route_token_segments(compact)
...
def _literal_value_has_raw_nested_security_pickle(
    ...
    *,
    ...
    for offset, marker in enumerate(value):
        ...
        if recovered_extension_context and _candidate_truncates_extension_operand(
...
def _raw_nested_extension_opcode_candidate_has_structural_signal(
    ...
    *,
    ...
    while search_start < search_limit:
        ...
        candidate = value[candidate_start : candidate_start + _MAX_RAW_NESTED_PICKLE_CANDIDATE_BYTES]
        candidate_is_prefix = candidate_start + len(candidate) < len(value)
        if _candidate_truncates_extension_operand(
            candidate_start=candidate_start,
            candidate=candidate,
            offset=offset,
            marker=value[offset],
