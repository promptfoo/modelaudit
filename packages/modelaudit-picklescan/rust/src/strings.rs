...
fn strip_url_spans_with_context(
    ...
    continuation: Option<StringScanContinuation>,
    next_window_start: usize,
) -> (Cow<'_, str>, bool, bool, Option<StringScanContinuation>) {
    ...
    while cursor < bytes.len() {
        ...
        let lex_stack_before_step = (step_start == next_window_start).then(|| lex_stack.clone());
        ...
        let step_changed_lexer = lex_stack_before_step
            .as_ref()
    ...
    if next_window_start == bytes.len() {
        next_continuation = Some(StringScanContinuation {
            lex_stack,
            inside_url: false,
            reviewed_prefix_bytes: 0,
...
mod tests {
    ...
    #[test]
    fn window_continuation_captures_prefixed_quote_starting_at_overlap_boundary() {
        ...
        for prefix in ["f", "F", "fr", "Fr", "fR", "rf", "rF", "RF"] {
            for quote in ["'", "\"", "'''", "\"\"\""] {
                ...
                for boundary in BOUNDARY..BOUNDARY + quote.len() {
                    ...
                    assert!(
                        matches.contains(&"os.system".to_string()),
                        "prefix={prefix:?} quote={quote:?} boundary={boundary} matches={matches:?}",
                    );
                }
            }
        }
    ...
    #[test]
    fn window_continuation_keeps_nonformatted_prefixed_url_payload_inert() {
        const BOUNDARY: usize = 64;
        for prefix in [
    ...
    fn window_continuation_does_not_skip_ordinary_boundary_base64_token() {
        ...
        let literal = format!("{}{encoded}", "!".repeat(BOUNDARY));
    ...
    #[test]
    fn window_continuation_preserves_url_starting_at_overlap_boundary() {
        const BOUNDARY: usize = 64;
        let literal = format!(
            "{}https://example.invalid/b3Muc3lzdGVtKCdpZCcp",
            "!".repeat(BOUNDARY),
        );
        let matches = scan_windows_split_inside_lexical_unit(&literal, BOUNDARY, BOUNDARY + 32);
    ...
    #[test]
    fn window_continuation_preserves_context_before_later_overlap_url() {
        const NEXT_WINDOW_START: usize = 8192 - 256;
        let url_suffix = format!("https://example.invalid/{};os.system(cmd)", "x".repeat(512));
        let literals = [
            format!(
                "metadata='{}{}'",
                "a".repeat(NEXT_WINDOW_START + 64 - "metadata='".len()),
                url_suffix
            ),
            format!(
                "metadata=f'{{func(\"{}{}\")}}'",
                "a".repeat(NEXT_WINDOW_START + 64 - "metadata=f'{func(\"".len()),
                url_suffix
            ),
            format!(
                "# {}{}\nmetadata = 1",
                "a".repeat(NEXT_WINDOW_START + 64 - "# ".len()),
                url_suffix
            ),
        ];
    ...
    #[test]
    fn window_continuation_preserves_code_after_later_overlap_url() {
        const NEXT_WINDOW_START: usize = 8192 - 256;
        let literal = format!(
            "metadata='{}https://example.invalid/{}';os.system(cmd)",
            "a".repeat(NEXT_WINDOW_START + 64 - "metadata='".len()),
            "x".repeat(512)
        );
