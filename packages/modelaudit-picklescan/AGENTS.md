...
`modelaudit-picklescan` is the Rust-backed pickle scanner that ships as an independent PyPI package. The root `modelaudit` wheel depends on it at runtime via a hard `modelaudit-picklescan>=0.1.7,<0.2.0` pin in the root `pyproject.toml`.
...
When this package reaches `0.2.0`, the root `pyproject.toml` `modelaudit-picklescan>=0.1.7,<0.2.0` constraint must be widened in the same PR, or the next `modelaudit` release will be uninstallable.
