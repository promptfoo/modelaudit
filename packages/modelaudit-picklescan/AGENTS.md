...
When this package reaches `0.2.0`, the root `pyproject.toml` `modelaudit-picklescan>=0.1.0,<0.2.0` constraint must be widened in the same PR, or the next `modelaudit` release will be uninstallable.
