    ...
    *,
    ...
    if external:
        ...
        if not missing_external:
            with open(safe_tmp_path / external_path, "wb") as f:
    ...
    path = safe_tmp_path / "model.onnx"
    onnx.save(model, str(path))
    return path


def create_python_onnx_model(safe_tmp_path: Path) -> Path:
    X = helper.make_tensor_value_info("input", TensorProto.FLOAT, [1])
    Y = helper.make_tensor_value_info("output", TensorProto.FLOAT, [1])
...
def test_onnx_scanner_can_handle(safe_tmp_path):
    model_path = create_onnx_model(safe_tmp_path)
    assert OnnxScanner.can_handle(str(model_path))


def test_onnx_scanner_basic_model(safe_tmp_path):
    model_path = create_onnx_model(safe_tmp_path)
    scanner = OnnxScanner()
...
def test_onnx_scanner_custom_op(safe_tmp_path):
    model_path = create_onnx_model(safe_tmp_path, custom=True)
    result = OnnxScanner().scan(str(model_path))
    assert any("custom operator" in i.message.lower() for i in result.issues)


def test_onnx_scanner_external_data_missing(safe_tmp_path):
    model_path = create_onnx_model(safe_tmp_path, external=True, missing_external=True)
    result = OnnxScanner().scan(str(model_path))
    assert any("external data file" in i.message.lower() for i in result.issues)


def test_onnx_scanner_corrupted(safe_tmp_path):
    model_path = create_onnx_model(safe_tmp_path)
...
def test_onnx_scanner_python_op(safe_tmp_path):
    model_path = create_python_onnx_model(safe_tmp_path)
    result = OnnxScanner().scan(str(model_path))
