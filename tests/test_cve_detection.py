...
from modelaudit.cve_patterns import analyze_cve_patterns, get_cve_attribution
...
class TestCVE202013092Detection:
    ...
    def test_detect_cve_2020_13092_basic_pattern(self, tmp_path):
        ...
        assert len(cve_detections) > 0, f"Should detect CVE-2020-13092. Issues found: {[i.message for i in result.issues]}"
    ...
    def test_detect_cve_2020_13092_joblib_with_system_call(self, tmp_path):
        ...
        assert any(
            "joblib" in issue.message.lower() or "joblib" in str(issue.details).lower()
            for issue in result.issues
        ), "Should detect joblib patterns"
        
        assert any(
            "os.system" in issue.message.lower() or "system" in issue.message.lower()
            for issue in result.issues
    ...
    def test_detect_cve_2020_13092_sklearn_pipeline_attack(self, tmp_path):
        ...
        sklearn_detections = [
            issue for issue in result.issues
            if "sklearn" in issue.message.lower() or "pipeline" in issue.message.lower()
            or "sklearn" in str(issue.details).lower() or "pipeline" in str(issue.details).lower()
        ]
...
class TestCVE202434997Detection:
    ...
    def test_detect_cve_2024_34997_basic_pattern(self, tmp_path):
        ...
        assert len(cve_detections) > 0, f"Should detect CVE-2024-34997. Issues found: {[i.message for i in result.issues]}"
    ...
    def test_detect_cve_2024_34997_cache_exploitation(self, tmp_path):
        ...
        assert any(
            "cache" in issue.message.lower() or "numpy" in issue.message.lower()
            for issue in result.issues
...
class TestCVEFalsePositivePrevention:
    ...
    def test_legitimate_sklearn_model_no_false_positive(self, tmp_path):
        ...
        critical_cve_detections = [
            issue for issue in cve_2020_detections
    ...
    def test_legitimate_joblib_usage_no_false_positive(self, tmp_path):
        ...
        cve_critical_issues = [
            ...
            if "CVE-" in issue.message or "CVE-" in str(issue.details)
        ]
...
class TestCVEPatternAnalysis:
    ...
    def test_no_cve_patterns_in_benign_content(self):
        ...
        assert len(attributions) == 0, f"Benign content should not trigger CVE detection. Found: {[a.cve_id for a in attributions]}"
...
class TestCVEIntegration:
    ...
    def test_joblib_scanner_cve_detection(self, tmp_path):
        ...
        dangerous_checks = [
            ...
            if ("CVE" in check.name or "CVE" in check.message or 
                "Dangerous" in check.name or 
                check.status.value == "failed")
        ]
        
        assert len(dangerous_checks) > 0, f"JoblibScanner should detect dangerous patterns. Found checks: {[c.name for c in result.checks]}"
    ...
    def test_pickle_scanner_cve_detection(self, tmp_path):
        ...
        dangerous_checks = [
            check for check in result.checks
            if (check.status.value == "failed" and 
                check.severity in [IssueSeverity.CRITICAL, IssueSeverity.WARNING])
        ]
        
        assert len(dangerous_checks) > 0, f"PickleScanner should detect dangerous patterns. Found checks: {[c.name for c in result.checks]}"
...
def test_cve_detection_with_existing_scanners():
    ...
    assert hasattr(joblib_scanner, '_scan_for_joblib_specific_threats'), "JoblibScanner should have specific threat detection"
