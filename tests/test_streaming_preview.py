...
def test_get_streaming_preview_detects_onnx(safe_tmp_path, monkeypatch):
    _setup_local(monkeypatch)
    file_path = safe_tmp_path / "model.onnx"
...
def test_get_streaming_preview_avoids_false_positive(safe_tmp_path, monkeypatch):
    _setup_local(monkeypatch)
    file_path = safe_tmp_path / "not_onnx.bin"
