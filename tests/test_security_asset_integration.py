    ...
    def test_malicious_sample_detection(self, samples_dir):
        ...
        for malicious_file in malicious_files:
            ...
            if not tensorflow_available and (
                "pyfunc" in malicious_file.name.lower()
                or "tensorflow" in str(malicious_file.parent).lower()
            ):
                skipped_files.append(
                    f"{malicious_file.name} (TensorFlow scanner unavailable)"
                )
                continue
            ...
            if exit_code == 0:
                ...
                if file_ext in [".pb"] and not tensorflow_available:
                    skipped_files.append(
                        f"{malicious_file.name} (required .pb scanner unavailable)"
                    )
                    continue
        ...
        if not tested_files and skipped_files:
            pytest.skip(
                f"All malicious files skipped due to scanner unavailability: {skipped_files}"
            )
    ...
    def test_safe_sample_validation(self, samples_dir):
        ...
        for safe_file in safe_files:
            ...
            assert exit_code in [
                0,
                1,
            ], f"Unexpected exit code {exit_code} for {safe_file.name}: {results['issues']}"
