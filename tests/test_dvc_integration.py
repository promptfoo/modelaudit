...
class TestDvcIntegration:
    ...
    def test_resolve_dvc_file_basic(self, safe_tmp_path):
        """Test basic DVC file resolution."""
        target = safe_tmp_path / "model.pkl"
        with target.open("wb") as f:
            pickle.dump({"a": 1}, f)

        dvc_file = safe_tmp_path / "model.pkl.dvc"
        dvc_file.write_text("outs:\n- path: model.pkl\n")

        resolved = resolve_dvc_file(str(dvc_file))
        assert resolved == [str(target.resolve())]

    def test_scan_dvc_pointer(self, safe_tmp_path):
    ...
    def test_directory_scan_expands_dvc(self, safe_tmp_path):
    ...
    def test_resolve_multiple_outputs(self, safe_tmp_path):
        """Test DVC file with multiple outputs."""
        # Create multiple targets
        targets = []
        for i in range(3):
            target = safe_tmp_path / f"model_{i}.pkl"
            with target.open("wb") as f:
        ...
        dvc_file = safe_tmp_path / "multi_model.dvc"
        dvc_file.write_text(dvc_content)

        resolved = resolve_dvc_file(str(dvc_file))
        assert len(resolved) == 3
        for target in targets:
            assert str(target.resolve()) in resolved

    def test_resolve_subdirectory_outputs(self, safe_tmp_path):
        """Test DVC file with outputs in subdirectories."""
        # Create subdirectory
        sub_dir = safe_tmp_path / "models"
        sub_dir.mkdir()
        ...
        dvc_file = safe_tmp_path / "nested.dvc"
        dvc_file.write_text("outs:\n- path: models/model.pkl\n")

        resolved = resolve_dvc_file(str(dvc_file))
        assert resolved == [str(target.resolve())]

    def test_missing_targets_ignored(self, safe_tmp_path):
        """Test that missing targets are ignored gracefully."""
        # Create one existing target
        existing = safe_tmp_path / "existing.pkl"
        with existing.open("wb") as f:
            pickle.dump({"exists": True}, f)

        dvc_file = safe_tmp_path / "partial.dvc"
        dvc_file.write_text("""outs:
- path: existing.pkl
- path: missing.pkl
...
class TestDvcSecurity:
    ...
    def test_path_traversal_prevention(self, safe_tmp_path):
        ...
        dvc_dir = safe_tmp_path / "dvc_project"
        dvc_dir.mkdir()
    ...
    def test_absolute_path_prevention(self, safe_tmp_path):
        ...
        dvc_file = safe_tmp_path / "absolute.dvc"
        ...
        assert str(target.resolve()) in resolved or resolved == []

    def test_symlink_traversal_prevention(self, safe_tmp_path):
        """Test prevention of symlink-based traversal."""
        # Create directories
        safe_dir = safe_tmp_path / "safe"
        safe_dir.mkdir()
        unsafe_dir = safe_tmp_path / "unsafe"
        unsafe_dir.mkdir()
    ...
    def test_resource_exhaustion_prevention(self, safe_tmp_path):
        ...
        dvc_file = safe_tmp_path / "excessive.dvc"
        dvc_file.write_text(dvc_content)
    ...
    def test_malformed_dvc_file_handling(self, safe_tmp_path):
        ...
        for content, description in test_cases:
            dvc_file = safe_tmp_path / f"malformed_{description.replace(' ', '_')}.dvc"
            dvc_file.write_text(content)

            resolved = resolve_dvc_file(str(dvc_file))
            assert resolved == [], f"Should handle {description} gracefully"

    def test_special_characters_in_paths(self, safe_tmp_path):
        ...
        for filename in special_files:
            file_path = safe_tmp_path / filename
        ...
        dvc_file = safe_tmp_path / "special_chars.dvc"
        dvc_file.write_text(dvc_content)

        resolved = resolve_dvc_file(str(dvc_file))
        assert len(resolved) == len(special_files)

    def test_non_dvc_file_ignored(self, safe_tmp_path):
        """Test that non-DVC files are ignored."""
        regular_file = safe_tmp_path / "not_dvc.txt"
        regular_file.write_text("outs:\n- path: something.pkl\n")

        resolved = resolve_dvc_file(str(regular_file))
        assert resolved == []

    def test_missing_yaml_dependency(self, safe_tmp_path, monkeypatch):
        ...
        dvc_file = safe_tmp_path / "test.dvc"
        dvc_file.write_text("outs:\n- path: model.pkl\n")

        resolved = resolve_dvc_file(str(dvc_file))
        assert resolved == []

    def test_dvc_with_malicious_pickle(self, safe_tmp_path):
        """Test that DVC integration doesn't bypass security scanning."""
        # Create a malicious pickle file
        malicious_pickle = safe_tmp_path / "malicious.pkl"
        ...
        dvc_file = safe_tmp_path / "malicious.dvc"
        dvc_file.write_text("outs:\n- path: malicious.pkl\n")
...
class TestDvcCliIntegration:
    ...
    def test_cli_dvc_file_expansion(self, safe_tmp_path):
