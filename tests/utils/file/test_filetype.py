...
def test_protocol0_spaced_persid_callback_precedes_eof() -> None:
    persistent_ids: list[str] = []
    ...
    with pytest.raises(EOFError):
        TrackingUnpickler(io.BytesIO(b"PMIT License\n")).load()

    assert persistent_ids == ["MIT License"]
...
def test_persid_callbacks_cover_line_and_binary_operands(payload: bytes, expected_id: str) -> None:
    persistent_ids: list[str] = []
    ...
    TrackingUnpickler(io.BytesIO(payload)).load()

    assert persistent_ids == [expected_id]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            b"MIT License\ncnotinstalled\nname\ncxx\n",
            PICKLE_ROUTING_INCONCLUSIVE_FORMAT,
            id="embedded-unknown-GLOBAL-before-newline-terminated-opcode-prefixed-tail",
        ),
        pytest.param(
            b"MIT License\ncnumpy\narray\n",
            PICKLE_ROUTING_INCONCLUSIVE_FORMAT,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(b"MIT License\n97\n", "pickle", id="one-byte-hex-NEXT_BUFFER"),
        ...
        pytest.param(
            b"MIT License\n" + base64.b64encode(b"cnotinstalled\nname\ncxx\n"),
            "pickle",
            id="base64-unknown-GLOBAL-before-newline-terminated-opcode-prefixed-tail",
        ),
        pytest.param(
            b"MIT License\n" + binascii.hexlify(b"cnotinstalled\nname\ncxx\n"),
            "pickle",
            id="hex-unknown-GLOBAL-before-newline-terminated-opcode-prefixed-tail",
        ),
        pytest.param(
            b"MIT License\nWFBpZAou\n",
            PICKLE_ROUTING_INCONCLUSIVE_FORMAT,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(b"2MIT License\n", id="DUP"),
        pytest.param(b"Pid\nMIT License\n", id="PERSID"),
        pytest.param(b"p0\nMIT License\n", id="PUT"),
        pytest.param(b"(S'unterminated", id="truncated-completable"),
    ],
)
def test_embedded_global_with_stack_valid_or_truncated_continuation_fails_closed(
    tmp_path: Path,
    continuation: bytes,
) -> None:
    path = tmp_path / "LICENSE"
...
def test_embedded_global_uses_valid_pre_global_stack_context(
    tmp_path: Path,
    pickle_stream: bytes,
) -> None:
    path = tmp_path / "LICENSE"
...
@pytest.mark.parametrize(
    "padding_size",
    [
        PROTO0_1_MAX_PROBE_BYTES - 2,
        PROTO0_1_MAX_PROBE_BYTES - 1,
        PROTO0_1_MAX_PROBE_BYTES,
        PROTO0_1_MAX_PROBE_BYTES + 1,
    ],
)
def test_embedded_global_at_lookbehind_boundary_fails_closed(
    tmp_path: Path,
    padding_size: int,
) -> None:
    path = tmp_path / "LICENSE"
...
@pytest.mark.parametrize(
    "operand_size",
    [PROTO0_1_MAX_PROBE_BYTES, PROTO0_1_MAX_PROBE_BYTES + 1, PROTO0_1_MAX_PROBE_BYTES + 128],
)
def test_embedded_global_recovers_protocol0_line_operand_boundary(
    tmp_path: Path,
    operand_size: int,
) -> None:
    path = tmp_path / "LICENSE"
...
def test_structured_xml_model_named_license_precedes_legal_text_fallback(
    tmp_path: Path,
    payload: bytes,
    expected_format: str,
) -> None:
...
def test_legal_named_pickle_with_post_stop_persid_fails_closed(tmp_path: Path) -> None:
    path = tmp_path / "LICENSE"
    path.write_bytes(b"S'MIT License'\n.Pdangerous\n")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(b"l", "pickle", id="LIST"),
        pytest.param(b"d", "pickle", id="DICT"),
        pytest.param(b"p0\n", "pickle", id="PUT"),
        pytest.param(b"g0\n", "pickle", id="GET"),
        pytest.param(b"b", "pickle", id="BUILD"),
    ],
)
def test_legal_named_pickle_with_protocol0_continuation_fails_closed(
    tmp_path: Path,
    continuation: bytes,
    expected_format: str,
) -> None:
    path = tmp_path / "LICENSE"
    path.write_bytes(b"cposix\nsystem\n" + continuation + b"MIT License\nCopyright Example\n")
...
def test_detect_file_format_does_not_text_route_malformed_or_misleading_legal_names(tmp_path: Path) -> None:
    invalid_utf8 = tmp_path / "LICENSE"
    invalid_utf8.write_bytes(b"MIT License\nCopyright \xe2\x82")
    control_bytes = tmp_path / "NOTICE"
    control_bytes.write_bytes(b"NOTICE\nCopyright\x00")
    misleading_pickle_suffix = tmp_path / "LICENSE.pkl"
    misleading_pickle_suffix.write_text("MIT License\nCopyright (c) Example\n", encoding="utf-8")

    assert detect_file_format(str(invalid_utf8)) != "text"
    assert detect_file_format(str(control_bytes)) != "text"
    assert detect_file_format(str(misleading_pickle_suffix)) != "text"


def test_detect_file_format_preserves_polyglot_and_oversized_legal_name_controls(tmp_path: Path) -> None:
    zip_named_license = tmp_path / "LICENSE"
    with zipfile.ZipFile(zip_named_license, "w") as archive:
