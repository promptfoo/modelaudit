...
def test_pytorch_binary_scanner_can_handle(safe_tmp_path):
    ...
    binary_file = safe_tmp_path / "model.bin"
    ...
    assert not scanner.can_handle(str(safe_tmp_path))

    # Should not handle other extensions
    other_file = safe_tmp_path / "model.txt"
    other_file.write_text("not a binary file")
    assert not scanner.can_handle(str(other_file))


def test_pytorch_binary_scanner_basic_scan(safe_tmp_path):
...
def test_pytorch_binary_scanner_code_patterns(safe_tmp_path):
    """Test detection of embedded code patterns."""
    scanner = PyTorchBinaryScanner()

    # Create a binary file with embedded code patterns
    binary_file = safe_tmp_path / "malicious.bin"
...
def test_pytorch_binary_scanner_executable_signatures_at_start(safe_tmp_path):
    ...
    binary_file = safe_tmp_path / "real_exe.bin"
...
def test_pytorch_binary_scanner_no_false_positive_mz(safe_tmp_path):
    ...
    binary_file = safe_tmp_path / "bert_weights.bin"
...
def test_pytorch_binary_scanner_longer_signatures_still_detected(safe_tmp_path):
    ...
    binary_file = safe_tmp_path / "with_elf.bin"
...
def test_pytorch_binary_scanner_blacklist_patterns(safe_tmp_path):
    ...
    binary_file = safe_tmp_path / "with_blacklist.bin"
...
def test_pytorch_binary_scanner_small_file(safe_tmp_path):
    """Test handling of suspiciously small files."""
    scanner = PyTorchBinaryScanner()

    # Create a very small binary file
    binary_file = safe_tmp_path / "tiny.bin"
...
def test_filetype_detection_for_bin_files(safe_tmp_path):
    ...
    pickle_bin = safe_tmp_path / "pickle.bin"
    ...
    safetensors_bin = safe_tmp_path / "safetensors.bin"
    ...
    onnx_bin = safe_tmp_path / "onnx.bin"
...
def test_pickle_scanner_handles_pickle_bin_files(safe_tmp_path):
    ...
    pickle_bin = safe_tmp_path / "model.bin"
    import pickle
