...
def test_assets_safetensors(safe_tmp_path: Path) -> None:
...
def test_assets_zip(safe_tmp_path: Path) -> None:
...
def test_assets_safetensors_metadata(safe_tmp_path: Path) -> None:
...
def test_assets_json_manifest_metadata(safe_tmp_path: Path) -> None:
    """Test that JSON manifest assets include keys metadata."""
    config_path = safe_tmp_path / "config.json"  # This filename will be recognized by ManifestScanner
    config_data = {
        "model_type": "bert",
        "hidden_size": 768,
...
def test_assets_pickle_file(safe_tmp_path: Path) -> None:
...
def test_assets_directory_scan_multiple_files(safe_tmp_path: Path) -> None:
    """Test that directory scanning captures all files as assets."""
    model_dir = safe_tmp_path / "model_dir"
    model_dir.mkdir()
...
def test_assets_structure_validation(safe_tmp_path: Path) -> None:
    """Test that asset structure follows the expected schema."""
    file_path = safe_tmp_path / "test.safetensors"
...
def test_assets_error_files_handling(safe_tmp_path: Path) -> None:
    ...
    assert unknown_asset.path == str(bad_file.resolve())

    # Good file should still be processed normally
    good_assets = [a for a in results.assets if a.path == str(good_file.resolve())]
    assert len(good_assets) == 1
    assert good_assets[0].type == "manifest"


def test_assets_empty_results(safe_tmp_path: Path) -> None:
...
def test_assets_file_type_detection(
    ...
    file_path = safe_tmp_path / f"test{file_extension}"
...
def test_assets_generic_json_detection(safe_tmp_path: Path) -> None:
    """Test that generic JSON files are detected as unknown type."""
    # Generic JSON file (not ML-specific)
    json_file = safe_tmp_path / "data.json"
