...
def test_tensorrt_scanner_can_handle(safe_tmp_path):
    path = safe_tmp_path / "model.engine"
    path.write_bytes(b"dummy")
    assert TensorRTScanner.can_handle(str(path))


def test_tensorrt_scanner_cannot_handle_wrong_extension(safe_tmp_path):
    path = safe_tmp_path / "model.txt"
    path.write_bytes(b"dummy")
...
def test_tensorrt_scanner_detects_suspicious_pattern(safe_tmp_path):
    path = safe_tmp_path / "malicious.engine"
...
def test_tensorrt_scanner_safe_file(safe_tmp_path):
    path = safe_tmp_path / "safe.engine"
