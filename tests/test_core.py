...
def test_scan_file_selected_jax_preserves_ambiguous_large_json(tmp_path: Path) -> None:
    generic_path = tmp_path / "generic.json"
    generic_path.write_text(
        json.dumps({"padding": "x" * (JAX_JSON_CHECKPOINT_ROUTING_READ_BYTES + 16)}),
        encoding="utf-8",
...
def test_scan_file_tokenizer_json_jinja_jax_xgboost_overlap_does_not_recurse(tmp_path: Path) -> None:
    tokenizer_path = _write_ordered_hf_tokenizer_json(
        tmp_path / "tokenizer.json",
        late_fields=(
            ',"learner":{"gradient_booster":{},"malicious_code":"os.system()"},'
    ...
    assert result.success is False
    assert set(result.metadata["scanner_dependency_ids"]) >= {"xgboost", "jax_checkpoint", "jinja2_template"}
    assert (
        sum(check.name == "JSON Content Analysis" and check.status == CheckStatus.FAILED for check in result.checks)
        == 1
    )
