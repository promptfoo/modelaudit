...
def test_tf_scanner_explanations_for_all_suspicious_ops(tmp_path):
    ...
    for op_name in SUSPICIOUS_OPS:
        ...
        model_dir = tmp_path / f"test_model_{op_name.lower()}"
        model_dir.mkdir()

        # Create SavedModel with suspicious operation
        saved_model = SavedModel()
        meta_graph = saved_model.meta_graphs.add()
        meta_graph.meta_info_def.tags.append("serve")

        # Add a node with the suspicious operation
        graph_def = meta_graph.graph_def
        node = graph_def.node.add()
        node.name = f"suspicious_{op_name.lower()}_node"
        node.op = op_name
        ...
        suspicious_issues = [
            issue
            for issue in result.issues
            if op_name.lower() in issue.message.lower() and issue.severity == IssueSeverity.CRITICAL
        ]
...
def test_tf_scanner_explanation_categories(tmp_path):
    ...
    for op_name in critical_ops:
        model_dir = tmp_path / f"critical_test_{op_name.lower()}"
        model_dir.mkdir()
        ...
        node = meta_graph.graph_def.node.add()
        node.name = f"critical_node_{op_name.lower()}"
        node.op = op_name
        ...
        op_issues = [issue for issue in result.issues if op_name.lower() in issue.message.lower()]
...
def test_tf_scanner_no_explanation_for_safe_ops(tmp_path):
    ...
    for i, op_name in enumerate(safe_ops):
        node = meta_graph.graph_def.node.add()
        node.name = f"safe_node_{i}"
        node.op = op_name
