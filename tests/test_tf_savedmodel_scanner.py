...
def test_tf_savedmodel_scanner_malicious_model(tmp_path):
    ...
    assert any("malicious.pkl" in issue.message.lower() or
...
def test_tf_savedmodel_scanner_invalid_model(tmp_path):
    ...
    assert any("variables" in issue.message.lower() or
...
def test_tf_savedmodel_scanner_with_blacklist(tmp_path):
    ...
    (model_dir / "custom_file.txt").write_bytes(b"This file contains suspicious_function")
    
    # Create scanner with custom blacklist
    scanner = TensorFlowSavedModelScanner(config={"blacklist_patterns": ["suspicious_function"]})
    result = scanner.scan(str(model_dir))
