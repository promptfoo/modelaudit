"""
Tests for graceful degradation and error handling functionality.

This module tests the enhanced error handling and graceful degradation features
...
from modelaudit.scanners import SCANNER_REGISTRY, _registry, get_scanner_for_file, ScannerRegistry
...
class TestErrorHandlingSpecificity:
    ...
    def test_import_error_vs_attribute_error(self):
        ...
        for scanner_id, error_msg in failed_scanners.items():
            ...
            has_import_info = ('dependencies' in error_msg or 
                             'import failed' in error_msg or 
                             'pip install' in error_msg)
            has_attribute_info = ('not found' in error_msg or 
                                'AttributeError' in error_msg)
            
            assert (has_import_info or has_attribute_info), \
                f"Error message should be specific: {error_msg}"
    ...
    def test_numpy_compatibility_error_detection(self):
        ...
        for error in numpy_errors:
            assert _is_numpy_compatibility_error(error), \
                f"Should detect NumPy error: {error}"
        ...
        for error in non_numpy_errors:
            assert not _is_numpy_compatibility_error(error), \
                f"Should not detect as NumPy error: {error}"
