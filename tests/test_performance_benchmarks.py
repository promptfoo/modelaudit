...
class TestPerformanceBenchmarks:
    ...
    def test_single_file_performance(self, assets_dir, performance_thresholds):
        ...
        for filename in test_files:
            ...
            if metrics["duration_stdev"] > 0:
                ...
                assert cv < 0.5, f"Performance too inconsistent (CV={cv:.2f}) for {filename}"
    ...
    def test_timeout_performance(self, assets_dir):
        ...
        start_time = time.time()
        results_long_timeout = scan_model_directory_or_file(str(assets_dir), timeout=300)
        duration_long = time.time() - start_time
        
        # Test with shorter but reasonable timeout  
        start_time = time.time()
        results_short_timeout = scan_model_directory_or_file(str(assets_dir), timeout=60)
        ...
        timeout_overhead = abs(duration_long - duration_short) / min(duration_long, duration_short)
        assert timeout_overhead < 0.1, f"Timeout mechanism adds too much overhead: {timeout_overhead:.2%}"
    ...
    def benchmark_and_save_results(self, assets_dir, output_file: str = "benchmark_results.json"):
