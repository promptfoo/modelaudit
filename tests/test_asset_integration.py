...
class TestAssetIntegration:
    ...
    def test_end_to_end_safe_validation(self, safe_assets):
        ...
        for asset_path in safe_assets:
            ...
            assert exit_code == 0, (
                f"False positive detected in {asset_path}: {results['issues']}"
            )
            assert results["success"] is True, f"Scan failed for {asset_path}"
            ...
            assert len(non_debug_issues) == 0, (
                f"Non-debug issues found in safe asset {asset_path}: {non_debug_issues}"
            )
    ...
    def test_cli_integration_malicious_directory(self, assets_dir, malicious_assets):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert result.exit_code == 1, (
                f"CLI should return exit code 1 for malicious content, got {result.exit_code}"
            )
            ...
            json_result = runner.invoke(
                cli, ["scan", str(temp_path), "--format", "json"]
            )
    ...
    def test_cli_integration_safe_directory(self, safe_assets):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert result.exit_code == 0, (
                f"CLI should return exit code 0 for safe content, got {result.exit_code}. Output: {result.output}"
            )
            assert "Files scanned:" in result.output
        ...
        for asset_name, result in results.items():
            if not result.get("skipped"):
                assert result["success"] is True, (
                    f"Concurrent scan failed for {asset_name}"
                )
        ...
        for asset_path in safe_assets:
            ...
            for issue in results["issues"]:
                ...
                has_false_positive = any(
                    pattern in message for pattern in false_positive_patterns
                )
                assert not has_false_positive, (
                    f"Potential false positive in {asset_path}: {message}"
                )
