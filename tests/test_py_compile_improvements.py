...
class TestPyCompileImprovements:
    ...
    def test_keras_lambda_false_positive_reduction(self, tmp_path):
        ...
        suspicious_lambda = [i for i in lambda_issues if "suspicious" in i.message.lower() or "configuration" in i.message.lower()]
...
class TestFalsePositiveMetrics:
    ...
    def test_measure_false_positive_reduction(self, tmp_path):
        ...
        for case in true_positive_cases:
            ...
            result = scanner.scan(str(pickle_path))
            executable_issues = [i for i in result.issues if "executable code" in i.message or "dangerous" in i.message.lower()]
            if executable_issues:
                new_true_positives += 1
        
        # Calculate metrics
        old_precision = old_true_positives / (old_true_positives + old_false_positives) if (old_true_positives + old_false_positives) > 0 else 0
        new_precision = new_true_positives / (new_true_positives + new_false_positives) if (new_true_positives + new_false_positives) > 0 else 0
        ...
        print(f"Old Scanner: {old_false_positives} false positives, {old_true_positives} true positives (precision: {old_precision:.2%})")
        print(f"New Scanner: {new_false_positives} false positives, {new_true_positives} true positives (precision: {new_precision:.2%})")
...
"""
        ...
        subprocess_detected = any(
            "subprocess" in issue.message.lower() or 
            ("details" in dir(issue) and "subprocess" in str(issue.details).lower())
