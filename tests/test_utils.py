...
def test_is_within_directory_simple(safe_tmp_path):
...
def test_is_within_directory_outside(safe_tmp_path):
    base_dir = safe_tmp_path / "base"
    base_dir.mkdir()
    outside = safe_tmp_path / "outside.txt"
    outside.write_text("data")
    assert is_within_directory(str(base_dir), str(outside)) is False


def test_is_within_directory_symlink_inside_to_outside(safe_tmp_path):
    base_dir = safe_tmp_path / "base"
    base_dir.mkdir()
...
def test_is_within_directory_symlink_outside_to_inside(safe_tmp_path):
    base_dir = safe_tmp_path / "base"
    base_dir.mkdir()
    inside_file = base_dir / "inside.txt"
    inside_file.write_text("data")
    link = safe_tmp_path / "outside_link.txt"
