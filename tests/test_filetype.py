...
def test_detect_file_format_directory(safe_tmp_path):
...
def test_detect_file_format_large_directory(safe_tmp_path):
    """Ensure detection short-circuits in directories with many files."""
    tf_dir = safe_tmp_path / "tf_large"
    tf_dir.mkdir()
    (tf_dir / "saved_model.pb").write_bytes(b"dummy content")
...
def test_detect_file_format_zip(safe_tmp_path):
    """Test detecting a ZIP file format."""
    # Create a ZIP file
    zip_path = safe_tmp_path / "archive.zip"
...
def test_detect_file_format_by_extension(safe_tmp_path):
...
def test_detect_file_format_hdf5(safe_tmp_path):
    """Test detecting HDF5 format by magic bytes."""
    # Create a file with HDF5 magic bytes
    hdf5_path = safe_tmp_path / "test.dat"
    hdf5_magic = b"\x89HDF\r\n\x1a\n"
...
def test_is_zipfile(safe_tmp_path):
    ...
    non_zip_path = safe_tmp_path / "not_a_zip.txt"
...
def test_detect_file_format_tar(safe_tmp_path):
    """Detect tar archives by signature without extra I/O."""
    tar_path = safe_tmp_path / "archive.tar"
...
def test_find_sharded_files_relative_path(safe_tmp_path, monkeypatch):
...
def test_detect_gguf_ggml_formats(safe_tmp_path):
    ...
    fake_gguf_path = safe_tmp_path / "fake.gguf"
...
def test_detect_ggml_variant_formats(safe_tmp_path):
...
def test_validate_file_type(safe_tmp_path):
    ...
    bad_gguf = safe_tmp_path / "bad.gguf"
    ...
    bin_zip = safe_tmp_path / "model.bin"
    ...
    bin_pickle = safe_tmp_path / "weights.bin"
    bin_pickle.write_bytes(b"\x80\x03" + b"pickle data")
    assert validate_file_type(str(bin_pickle)) is True


def test_detect_file_format_from_magic_oserror(safe_tmp_path, monkeypatch):
    """Return 'unknown' when reading magic bytes fails."""
    file_path = safe_tmp_path / "unreadable.bin"
