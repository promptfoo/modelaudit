...
class TestSBOMURLMapping:
    ...
    def test_multiple_url_mappings(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Test data
            original_urls = ["hf://user/model/model.safetensors", "hf://user/model/config.json"]
    ...
    def test_nonexistent_path_handling(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            original_urls = ["hf://user/model/real.bin", "hf://user/model/nonexistent.bin"]
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            results_dict = {
                "file_metadata": {
                    ...
                    str(tokenizer_file): {"is_model": False, "file_size": len(b'{"vocab_size": 32000}')},
                },
                "issues": [],
                "checks": [],
            }
            ...
            safetensors_comp = next(comp for comp in sbom["components"] if comp["name"] == "model.safetensors")
