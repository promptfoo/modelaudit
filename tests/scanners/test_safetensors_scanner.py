...
def test_valid_zero_element_safetensors_tensor(tmp_path: Path) -> None:
    file_path = tmp_path / "zero-element.safetensors"
    write_raw_safetensors(
        file_path,
...
def test_valid_zero_element_tensor_after_nonempty_header_entry(tmp_path: Path) -> None:
    file_path = tmp_path / "mixed-zero-element.safetensors"
    write_raw_safetensors(
        file_path,
        {
            "nonempty": {"dtype": "F32", "shape": [1], "data_offsets": [0, 4]},
...
def test_zero_length_range_for_nonempty_tensor_is_critical(tmp_path: Path) -> None:
    file_path = tmp_path / "invalid-zero-length.safetensors"
    write_raw_safetensors(
        file_path,
        {"tensor": {"dtype": "F32", "shape": [1], "data_offsets": [0, 0]}},
        b"",
    )
