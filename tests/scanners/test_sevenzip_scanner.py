...
class TestSevenZipScannerHardening:
    ...
    def test_extensionless_probe_limit_preserves_observed_security_finding(self, tmp_path: Path) -> None:
        ...
        with py7zr.SevenZipFile(archive_path, "w") as archive:
            archive.writestr(pickle.dumps(DangerousPayload(), protocol=0), "first_payload")
        ...
        assert any(issue.severity in {IssueSeverity.WARNING, IssueSeverity.CRITICAL} for issue in aggregate.issues)
