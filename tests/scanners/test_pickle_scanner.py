...
class TestPickleScannerBlocklistHardening(unittest.TestCase):
    ...
    def test_pil_global_reduce_is_not_suppressed(self) -> None:
        ...
        reduce_checks = [c for c in result.checks if c.name == "REDUCE Opcode Safety Check"]
        assert any(c.details.get("associated_global") == "PIL.Image" for c in reduce_checks), (
            f"Expected PIL.Image to reach REDUCE analysis, got: {[c.details for c in reduce_checks]}"
        )
    ...
    def test_mixed_case_dangerous_builtin_import_only_still_flags_import(self) -> None:
        """Mixed-case dangerous builtins should keep the existing import-only signal."""
    ...
    def test_mixed_case_unknown_import_only_remains_non_failing(self) -> None:
        """Non-dangerous mixed-case import-only refs stay clean until broader import-only work lands."""
        ...
        assert not any(
            ...
            for check in result.checks
        ), f"Unexpected failed import-only check for EvilPkg.thing: {result.checks}"
