...
def create_external_link_weights_h5(tmp_path: Path, *, external_filename: str | None = None) -> Path:
    ...
    if external_filename is None:
        ...
        external_filename = external_source.name

    weights_path = tmp_path / "model.weights.h5"
    with h5py.File(weights_path, "w") as f:
        f["linked_kernel"] = h5py.ExternalLink(external_filename, "/payload")
...
class TestKerasZipScanner:
    ...
    def test_embedded_hdf5_evidence_redacts_plain_path_capabilities(self, tmp_path: Path) -> None:
        """Embedded HDF5 references should not serialize filesystem capability tokens."""
        ...
        keras_path = create_configured_keras_zip(
            ...
            {"class_name": "Sequential", "config": {"layers": []}},
            keras_version="3.12.0",
            weights_h5_path=create_external_link_weights_h5(
                tmp_path,
                external_filename=f"/cache/{capability}/weights.h5",
            ),
        )
    ...
    def test_lambda_code_preview_redacts_secret_bearing_evidence(self, tmp_path: Path) -> None:
        ...
        assert "ZIPMULTILINESECRET789" not in serialized
...
"""
    ...
    def test_stringlookup_vocabulary_evidence_redacts_plain_path_capabilities(self, tmp_path: Path) -> None:
                ...
                "layers": [
                    {
                        "class_name": "StringLookup",
                        "name": "string_lookup",
                        "config": {"vocabulary": f"/cache/{capability}/vocab.txt"},
                    },
                ],
            },
        }
        ...
        assert len(cve_checks) == 1
        assert capability not in serialized
        assert cve_checks[0].details["vocabulary"] == "/cache/<redacted>/vocab.txt"
