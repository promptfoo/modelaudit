...
class TestContextAwareFiltering:
    ...
    def test_should_ignore_tokenizer_specific_keys(self):
        ...
        for key, value, matches in tokenizer_keys:
            result = scanner._should_ignore_in_context(key, value, matches, tokenizer_context)
    ...
    def test_should_not_ignore_non_ml_context(self):
        ...
        for key, value, matches in suspicious_keys:
            result = scanner._should_ignore_in_context(key, value, matches, non_ml_context)
...
class TestSeverityAssignment:
    ...
    def test_execution_patterns_always_error(self):
        ...
        severity_non_ml = scanner._get_context_aware_severity(execution_matches, non_ml_context)
    ...
    def test_file_access_downgraded_in_ml_context(self):
        ...
        severity_high_ml = scanner._get_context_aware_severity(file_access_matches, high_confidence_ml)
        severity_low = scanner._get_context_aware_severity(file_access_matches, low_confidence_context)
    ...
    def test_network_access_downgraded_in_ml_context(self):
        ...
        severity = scanner._get_context_aware_severity(network_matches, high_confidence_ml)
...
class TestIntegrationScenarios:
    ...
    def test_dangerous_config_still_detected(self):
        ...
        try:
            ...
            dangerous_value_errors = [
                e for e in errors 
                if e.details.get('analysis') == 'value_based'
            ]
    ...
    def test_mixed_safe_and_dangerous_content(self):
        ...
        try:
            ...
            dangerous_errors = [
                e for e in errors 
                if 'init_script' in e.message or e.details.get('analysis') == 'value_based'
            ]
...
@pytest.mark.parametrize("framework,config_keys,expected_framework", [
...
def test_framework_detection_parametrized(scanner, framework, config_keys, expected_framework):
...
@pytest.mark.parametrize("dangerous_value,should_detect", [
    ("import os; os.system('rm -rf /')", True),
...
def test_dangerous_value_detection_parametrized(scanner, dangerous_value, should_detect):
