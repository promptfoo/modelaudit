...
| NeMo                            | `.nemo`, signature-valid renamed archives                         | Yes (static tar/config analysis, Hydra `_target_` checks) | None                                                                                             |
...
- Renamed NeMo archives require TAR structure plus a canonical `model_config.yaml` or `model_config.yml` member before Hydra `_target_` analysis is selected; NeMo scanning also retains generic TAR embedded-member checks.
