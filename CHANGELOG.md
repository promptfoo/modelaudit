...
- validate index-driven zero- and one-based SafeTensors shard families, retain case-insensitive or prefixed index authority after remote materialization, intersect same-parent indexes with selected shard families, bind streamed or explicit scans to one index generation, cap remote index reconciliation before range I/O, bind local directory dispatch to one filesystem object, and reject recreated delete-after-scan shards without widening selected scanners to foreign shard formats, suppressing overlap-capable routes, trusting names without index authority, exposing transient descriptor paths, or accepting post-pin source changes
