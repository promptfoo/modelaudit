...
- validate index-driven zero- and one-based SafeTensors shard families, retain case-insensitive, prefixed, or trusted ancestor index authority after remote materialization and across complete same-shape families, intersect same-parent indexes with selected shard families, bind streamed or explicit scans to one index generation, stop over-limit remote indexes before range or SDK I/O, retain standard and streamed local-directory descriptor chains, bind streamed reads to retained descriptors, reject pre-open symlink-target and Hugging Face blob generation changes, preserve generic hash-failure logs, keep Windows replacement guards while allowing ordinary child metadata changes, and reject recreated delete-after-scan shards without widening selected scanners to foreign shard formats, suppressing overlap-capable routes, trusting names without index authority, exposing transient descriptor paths, or accepting post-pin source changes
...
- preserve declared ONNX external data sidecars during standard filtered and streaming Hugging Face scans
