...
- fail closed on standalone Keras HDF5 scans when `h5py` is unavailable, including extensionless user-block files, without reviving stale cached results
- fail closed on standalone Keras HDF5 scans when `h5py` is unavailable, including user-block files, without reviving stale cached results
