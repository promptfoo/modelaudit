...
- **security:** recurse into embedded members of `.skops` archives while preserving Skops-specific CVE checks, avoiding tiny nested `.bin` false positives on clean archive members, and preserving CLI `scanner_names` in aggregated JSON output
